...
function findConsqSums(nums, targetSum) {
    ...
    while (littleIdx < nums.length) {
        if (nums[littleIdx] + nums[bigIdx] < targetSum) {
            checkerArr.push(nums[bigIdx])
            bigIdx++
        }else if(nums[littleIdx] + nums[bigIdx] === targetSum){
            checkerArr.push(nums[bigIdx])
            expectedArr.push(checkerArr)
            checkerArr = []
            littleIdx++
            bigIdx = littleIdx
        }else{
